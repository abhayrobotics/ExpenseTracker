...
- ✅ Add Expense# Expense Tracker — Architecture Reference

Internal engineering doc for future revisions. Not meant for the public README — this is the "how it works and what's next" reference for me.

---

## 1. System Overview

Standard three-tier CRUD app with per-user data isolation (each row scoped by `userId` — not multi-tenancy in the SaaS sense, just row-level filtering).

- **Client**: React 19 SPA (Vite, Tailwind CSS v4)
- **Server**: Node.js + Express 5, stateless JWT auth
- **Database**: PostgreSQL (Neon), accessed via Prisma 6
...
Keep these separate from "what exists" — they're ideas, not shipped architecture.
